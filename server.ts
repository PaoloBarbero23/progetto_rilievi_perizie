            ...
            collection.findOne(query)
                .then((dbUser) => {
                    if (!dbUser) {
                        ...
                        if (tipo == "utente") {//si è fatto l'accesso dall'applicazione
                        ...
                        else {//si è fatto l'accesso dal sito
                ...
                bcrypt.hash(req.body.password, 10, (err: Error, hash: string) => {
                    if (err) {
                        ...
                        collection.insertOne(req.body).then((data: any) => {
                            let msg = message.replace('__user', req.body.mail).replace("__password", pwd).replace("__content", "Benvenuto in Rilievi e Perizie, il tuo account è stato creato con successo.");
            ...
            bcrypt.hash(req.body.password, 10, (err: Error, hash: string) => {
                if (err) {
                    ...
                    collection.insertOne(record).then((data: any) => {
                        let msg = message.replace('__user', req.body.mail).replace("__password", req.body.password).replace("__content", "Benvenuto in Rilievi e Perizie, il tuo account è stato creato con successo.");
    ...
    bcrypt.hash(password, 10, (err, hash) => {
        if (err) {
            ...
            collection.updateOne({ mail: mail }, { $set: { password: hash, "change" : true } }).then((data: any) => {
                //manda mail
                let msg = message.replace('__user', mail).replace("__password", password).replace("__content", "La tua password è stata rigenerata.")
...
app.post("/api/getUser", (req: any, res: Response, next: NextFunction) => {
    ...
    collection.findOne({ _id: oId }, { projection: { "_id": 0 } }).then((data: any) => {
        //controllo se la password dell'utente è "password" e se è così la cambio
...
app.post("/api/uploadCloudinary", (req: any, res: Response, next: NextFunction) => {
    ...
    for (let i = 0; i < req.body.foto.length; i++) {
        cloudinary.v2.uploader.upload(req.body.foto[i], { folder: "progetto_rilievi_perizie" }).then((result: any) => {
            ...
            cont++;
            if (cont == req.body.foto.length)//se è l'ultima foto
            {
